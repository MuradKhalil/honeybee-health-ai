...
def load_img(path):
    # image_data = ImageDataSet(filepath=path)
    # img = image_data.load()
...
def load_bee_detection_model():
    detector = hub.load('data/06_models/faster_rcnn_openimages_v4_inception_resnet_v2_1').signatures['default']
    return detector

# detect objects
def run_detector(img_path, detector=None, display=False):
    img = load_img(img_path)
    start_time = time.time()
    if detector==None:
        detector = load_bee_detection_model()
...
def filter_bees(result, min_score=0.6):
